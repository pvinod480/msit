...
class Employee:
    ...
    def __init__(self):
        ...
        self.conn.execute("create table if not exists records(id text primary key unique ,name text,gender char(1),age INTEGER,salary INTEGER);")
    ...
    def addEmployee(self,id,name,gender,age,salary):
        ...
        if id not in self.empid:
            self.conn.execute("insert into records(id,name,gender,age,salary) values(?,?,?,?,?)", (id,name,gender,age,salary))
            self.conn.commit()
    ...
    def modifyEmployee(self,id,name,gender,age,salary):
        ...
        if id in self.empid :
            self.conn.execute("update records set name=?,gender=?,age=?,salary=? where id = ?", (name,gender,age,salary,id))
            self.conn.commit()
...
#e1.deleteEmployee("E009")
#e1.modifyEmployee("E005","varun","m",22,15000)
e1.dataInXml("E001")
e1.show()
