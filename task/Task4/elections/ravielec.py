...
nwb.remove(sheet)
rows = sheet.max_row
for data1 in range(rows):
        new_sheet.cell(row=data1+1,column=1).value=w_name
        new_sheet.cell(row=data1+1,column=2).value=r_name
        new_sheet.cell(row=data1+1,column=3).value=margin
nwb.save("Electionsmodify1.xlsx")
